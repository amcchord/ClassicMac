...
export default class RFB extends EventTargetMixin {
    constructor(target, urlOrChannel, options) {
        ...
        this._relativePointer = false;
        this._classicInputHelpers = false;
        this._classicRightClickActive = false;
    ...
    // ClassicMac additions. QEMU's Cocoa frontend previously supplied these
    // conveniences; the browser display keeps the same guest-facing behavior.
    get classicInputHelpers() { return this._classicInputHelpers; }
    set classicInputHelpers(enabled) {
        this._classicInputHelpers = enabled;
        if (!enabled && this._classicRightClickActive) {
    ...
    _handleMouse(ev) {
        ...
        let pos;
        if (this._relativePointer) {
            const center = 0x7fff;
            pos = {
                'x': Math.max(0, Math.min(0xffff, center + (ev.movementX || 0))),
                'y': Math.max(0, Math.min(0xffff, center + (ev.movementY || 0))),
            };
        } else {
        ...
        const classicRightDown = this._classicInputHelpers && (bmask & 0x4) !== 0;
        if (classicRightDown && !this._classicRightClickActive) {
            this.sendKey(KeyTable.XK_Control_L, "ControlLeft", true);
            this._classicRightClickActive = true;
        }
        if (classicRightDown) {
            bmask = (bmask & ~0x4) | 0x1;
        }
        ...
        switch (ev.type) {
            ...
            case 'mousemove':
                if (this._relativePointer) {
                    // Relative coordinates are deltas around 0x7fff in QEMU's
                    // pointer-type-change extension. Send each browser event;
                    // the normal absolute-position coalescer would discard
                    // intermediate deltas instead of accumulating them.
                    this._mouseButtonMask = bmask;
                    this._sendMouse(pos.x, pos.y, bmask);
                    break;
                }
        ...
        if (!classicRightDown && this._classicRightClickActive) {
            // Release Control after the mapped left button has gone up.
            this.sendKey(KeyTable.XK_Control_L, "ControlLeft", false);
            this._classicRightClickActive = false;
        }
    }
    ...
    _sendMouse(x, y, mask) {
        ...
        const wireX = this._relativePointer ? x : this._display.absX(x);
        const wireY = this._relativePointer ? y : this._display.absY(y);

        if (this._extendedPointerEventSupported && extendedMouseButtons) {
            RFB.messages.extendedPointerEvent(this._sock, wireX, wireY, mask);
        } else {
            RFB.messages.pointerEvent(this._sock, wireX, wireY, mask);
        }
    }
    ...
    _handleWheel(ev) {
        ...
        if (this._classicInputHelpers) {
            if (Math.abs(this._accumulatedWheelDeltaY) >= WHEEL_STEP) {
                if (this._accumulatedWheelDeltaY < 0) {
                    this.sendKey(KeyTable.XK_Up, "ArrowUp");
                } else {
                    this.sendKey(KeyTable.XK_Down, "ArrowDown");
                }
                this._accumulatedWheelDeltaY = 0;
            }
            this._accumulatedWheelDeltaX = 0;
    ...
    _sendEncodings() {
        ...
        encs.push(encodings.pseudoEncodingQEMUPointerTypeChange);
    ...
    _handleRect() {
        switch (this._FBU.encoding) {
            ...
            case encodings.pseudoEncodingQEMUPointerTypeChange:
                this._setRelativePointer(this._FBU.x === 0);
                return true;
    ...
    _setRelativePointer(relative) {
        if (this._relativePointer === relative) {
            return;
        }
        this._flushMouseMoveTimer(0x7fff, 0x7fff);
        this._relativePointer = relative;
        this._mousePos = {};
        this.dispatchEvent(new CustomEvent(
            "pointertypechange",
            { detail: { relative: this._relativePointer } }));
    }
