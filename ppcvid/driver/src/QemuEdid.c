...
/* Append a mode at the very end of the list without sorting or duplicate
   filtering. Used for the dynamic (host-resize) modes, whose display mode
   IDs must stay stable while their geometry is retargeted at runtime. */
void appendVModeToList(struct _vMode *vMode)
{
    struct vMode **v = &vModes;
    struct vMode *n;
