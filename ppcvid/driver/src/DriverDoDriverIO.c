...
DriverStatusCmd( IOCommandID ioCommandID, IOCommandKind ioCommandKind, CntrlParam *pb )
{
	...
	case cscQemuVgaGetHostResize: {
		/* Private selector for a task-time guest agent: report the current
		 * host resize request and the (retargeted) mode ID to switch to,
		 * so the agent can call DMSetDisplayMode itself. */
		QemuVgaHostResizeRec *rec = (QemuVgaHostResizeRec *) genericPtr;
		struct _vMode *cur = getVMode(GLOBAL.curMode);

		if (rec == NULL)
			return paramErr;
		rec->available = GLOBAL.hostResizeAvail ? 1 : 0;
		rec->serial = 0;
		rec->width = 0;
		rec->height = 0;
		rec->modeID = 0;
		rec->depthMode = 0;
		rec->curWidth = cur ? cur->width : 0;
		rec->curHeight = cur ? cur->height : 0;
		if (GLOBAL.hostResizeAvail) {
			rec->serial = QemuVga_ReadExt(QEMU_EXT_REG_REQ_SERIAL);
			rec->width = QemuVga_ReadExt(QEMU_EXT_REG_REQ_WIDTH);
