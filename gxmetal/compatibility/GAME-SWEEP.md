...
The strict schema-2 evidence audit on 2026-08-28 found **0 of 10 fully
qualified** at its starting snapshot. Every title has useful accelerated
rendering evidence, but a full pass also requires one reviewed route containing
representative gameplay/effects, clean application exit, same-boot second
launch, fresh renderer generation, muted audio, disabled networking, immutable
source proof, and fault-free direct presentation. Unreal Tournament was the
closest, missing only the five-minute route; the other rows below deliberately
describe their strongest partial scope rather than a universal pass.
...
| `cro-mag-rally` | Cro-Mag Rally Demo | Apple OpenGL 1.1.2 | Confirm hardware/OpenGL; complete a lap with terrain, particles, HUD, transparency, and camera transitions | Post-2.2.1 640x480 Practice/Desert race pass: coherent terrain/vehicle/HUD, steering deltas 0.857201 and 0.780189, and a 35-second soak. The retained post-Command-Q frame is still the race, so clean exit/relaunch and a complete lap remain open |
...
| `havoc` | Havoc Demo | First shipping QuickDraw 3D RAVE game | Select accelerated rendering; enter the demo arena; verify terrain, fog, textured objects, transparency, HUD, and camera motion for five minutes | Saved-hardware evidence preserves 48 pre-context textures and isolates clipped sentinel triangles. The 469.1-second run produces 45,094 direct/0 fallback frames and 5,940,035 draws, but returns to help/main-menu screens before the nominal five-minute endpoint; a separate HUD-gated route proves a 0.782257 input delta. The checked-in 15-sortie route now fails closed on 310.5 seconds of HUD-bracketed cockpit gameplay, clean exit, and relaunch, but still requires a live reviewed pass |
