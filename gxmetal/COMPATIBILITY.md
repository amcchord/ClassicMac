...
| Textures | Alpha1, RGB8_332, RGB555, RGB565, ARGB1555, ARGB4444, RGB32, ARGB32, CL4, CL8, ACL16_88, I8, and AI16_88; mipmaps; repeat/clamp; independent per-unit MIN/MAG state with nearest, bilinear, and trilinear filtering | Supported | Apple Software RAVE oracle, exact primary/secondary sampler-state regressions, distinct-color native and accelerated-AGL mip readback, asymmetric upload/sample, packed-nibble and byte-layout, channel-expansion, byte-order, per-pixel alpha, transparent-index, odd-width and row-padding tests, and classic-game runs |
...
| ATI private bridge | OpenGLRendererATI hooks, two-stage lightmaps, NoCopy refresh, narrow in-place slot-2 base-image replacement, one-shot slot-4 draw-buffer readback, pointer-fan clip markers, and private ARGB4444 | Supported compatibility path; proven slot-2 calls use flags zero, one level, and the existing resource's exact format and dimensions while preserving its identity; private readback returns the complete 24-byte `TQADevice` without leaving an access latch; zero and nonzero slot-60 markers submit valid fans; and private ARGB4444 accepts both observed V conventions | Myth II transition oracle, Quake III Q3DM1 missing-world oracle, Oni transition oracle, Carmageddon II, Future Cop, in-guest ATI update/readback conformance, native ATI tests, and host clamp-gradient tests |
...
| Perspective and unused Z | `kQATag_PerspectiveZ`, no-Z textured draws | Positive reciprocal-W is mapped monotonically over its full finite range for hidden-surface removal; textured draws with depth and fog both disabled accept finite legacy eye depth and normalize only out-of-range Metal position Z, preserving valid normalized Z for active fog; active depth or fog still requires `[0,1]` | Host textured and guest Gouraud tests make normalized Z and reciprocal W disagree, plus Myth II's exact 224-byte no-Z textured-fan and no-Z textured-fog regressions |
...
2. Complete or narrow extended OpenGL semantics. In particular, add point/line
   polygon modes and either implement stipple behavior or document why the
   system GLD never exposes it to applications.
...
- Future Cop: ATI-private ARGB4444 drawing with nonnegative V coordinates,
  source-color blending, clamped textures, HUD/effects, and live movement and
  combat input.
