...
| --- | --- | --- | --- | --- |
| 1 | Ready-to-run OS 9 download | Download Mac OS 9 with GXMetal, choose a name, and start; skip guest installation. | Medium–large | Implemented |
...
| 4 | Cleaner machine home | Lead with a desktop preview, Start, OS identity, and useful actions; move the long configuration form into Settings. | Medium | Implemented |
...
| 7 | Visible GXMetal status | Compact activity indicator and test/repair guidance; installed guest version is not exposed by the current protocol. | Medium–large | Implemented |
...
| 10 | Consistent media drawer | Inserted and recent discs, startup selection, and clear live-change versus next-start behavior. | Medium | Implemented |
...
| 13 | Paste text into guest | An explicit Paste Text into Mac command; full two-way clipboard is a later feature. | Medium–large | Implemented |
...
See the [3.0 validation record](3.0-validation.md),
[current state](progress/CURRENT.md), and [session journal](progress/JOURNAL.md)
for implementation, artifact identities, test evidence, and remaining work.
